...
export interface AuthControllerResponse { success: boolean; user?: any; message?: string }
...
export default class AuthController {
	...
	static async login(loginPayload: LoginDTO): Promise<AuthControllerResponse> {
		try {
			...
			if (!Bcrypt.compare(password, user.password)) return { success: false, message: 'Email and/or password invalid!' }
